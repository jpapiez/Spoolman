...
export function BulkImportFilaments(): React.ReactElement {
  ...
  const handleBulkImport = async () => {
    ...
    try {
      ...
      if (failCount === 0) {
        messageApi.success(
          t("notifications.importSuccessful", { count: successCount }) ||
            `Successfully imported ${successCount} filaments`,
        );
        // Clear selection after successful import
        setSelection({});
      } else {
        messageApi.warning(`${successCount} filaments imported successfully, ${failCount} failed`);
      }
      ...
      messageApi.error(error instanceof Error ? error.message : t("notifications.importFailed") || "Import failed");
    } finally {
      setImporting(false);
    }
  };
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        <Paragraph>{t("settings.importexport.bulkImportFilaments.description")}</Paragraph>
        ...
        <Table
          ...
          locale={{
            emptyText:
              filteredFilaments.length === 0 && externalFilaments.length > 0
